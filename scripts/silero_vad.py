#!/usr/bin/env python3
"""
Silero VAD stage: Voice Activity Detection using Silero
"""
import warnings
...
if __name__ == "__main__":
    ...
    stage_io = StageIO("silero_vad")
    logger = get_stage_logger("silero_vad", log_level="DEBUG", stage_io=stage_io)
    
    logger.info("=" * 60)
    logger.info("SILERO VAD STAGE: Voice Activity Detection")
    logger.info("=" * 60)
    ...
    device = os.environ.get("SILERO_DEVICE", 
    ...
    logger.info("Running Silero VAD chunker...")
    ...
    if exit_code is None:
        logger.error("✗ Silero VAD returned None - treating as failure")
        exit_code = 1
    
    if exit_code == 0:
        logger.info("✓ Silero VAD completed successfully")
    else:
        logger.error(f"✗ Silero VAD failed with exit code {exit_code}")
    
    logger.info("=" * 60)
    logger.info("SILERO VAD STAGE COMPLETED")
    logger.info("=" * 60)
